...
namespace Services.Article
{
    public class CommentService : ICommentService
    {
        ...
        {
            if (articleId < 1 || email.IsNullOrEmpty() || textComment.Length>50)
            {
                Log.Error("Invalid comments parameters: id={0}, textComment={1}, email={2}",
                    articleId, textComment,email);
                
                throw new ArgumentException("Invalid comments parameters");
            }
            ...
            if (await _unitOfWork.Articles.GetAsQueryable().Where(x=>x.Id== articleId).AnyAsync())
            {
                ...
                throw new ArgumentException("user does not exist:{0}",nameof(email));
            }
            else
            {

                throw new ArgumentException("article does not exist", nameof(articleId));
            }
        }
    }
}
